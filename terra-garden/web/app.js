...
$('plantMe').addEventListener('click', async () => {
    ...
    const id   = normId($('code').value);
    if(!name){ toast('Please enter your name 🌱'); return; }
    if(!ID_RE.test(id)){ toast('Bracelet code is letters/numbers, e.g. A1'); return; }
...
(async function init(){
    ...
    if(met){
        try {
            ...
            if(!snap.exists()){
                $('code').value = met;
                $('codeHint').textContent = 'Tapped your own bracelet? Its code ' + met + ' is filled in. Tapped a friend\'s? Enter yours instead.';
            } else {
                pendingMet = met;
